...
def zebra_framework(plo_plot, scr_path, key_path, label='ZEBRA profile',
    ...
    # print outs
    logger.log(utils.logging.NOTICE, "%s" % label)
    logger.log(utils.logging.NOTICE, "Population: %s bit" % str_dece)
    logger.log(utils.logging.NOTICE, "Individual: %s (%s)" % (str_max_abs_llr, cat_tag))
    ...
    if color_min is not None:
        ...
        if dcf_pot:
            ...
            logger.log(utils.logging.NOTICE, "1 - min Cllr: %.3f (0 is good)" % plo_plot.get_delta_DCF())
